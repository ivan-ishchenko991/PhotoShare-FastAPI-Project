...
async def transform_image(photo_id: int, body: TransformBodyModel, user: User, db: Session) -> Photo | None:
    """
    The transform_image function takes in a photo_id, body, user and db. It then initializes the cloudinary library.
    Next it queries the database for a photo that matches both the id and user_id of what was passed into this
    function. If there is no matching photo found, None is returned to indicate that nothing was found.
...
async def create_link_transform_image(photo_id: int, user: User, db: Session) -> str | None:
    """
    The create_link_transform_image function takes in a photo_id, user and db as parameters. It then initializes the
    cloudinary library. It then queries the database for a photo with that id and user_id. If it finds one,
    it creates a QR code from the image transform url of that photo using qrcode library. Then it uploads this QR
    code to Cloudinary under PhotoshareApp_tr folder with public id being original public id + 'qr'. It returns an
    object containing both image transform url and qr transform url.
